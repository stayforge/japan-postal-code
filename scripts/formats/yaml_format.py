...
async def save_yaml(data: List[JapanPostalCode], output_path: Path) -> None:
    ...
    try:
        ...
        task_id = None
        if progress_mgr:
            task_id = progress_mgr.add_task("all_data.yaml", total=1)
        ...
        if progress_mgr and task_id is not None:
            progress_mgr.update("all_data.yaml", advance=1)
...
async def save_grouped_yaml(grouped_data: Dict[str, Dict[str, List[JapanPostalCode]]], datasets_dir: Path) -> None:
    """Save grouped data as YAML files with hierarchical structure."""
    ...
    task_id = None
    if progress_mgr:
        task_id = progress_mgr.add_task("YAML", total=total_files)
    
    async def save_single_yaml_file(file_path: Path, yaml_data):
        ...
        async with semaphore:
            ...
            saved_files += 1
            if progress_mgr and task_id is not None:
                progress_mgr.update("YAML", advance=1)
    ...
    for prefix, suffix_dict in grouped_data.items():
        ...
        prefix_file = datasets_dir / f"{prefix}.yaml"
        prefix_data = [record.model_dump() for record in all_prefix_records]
        tasks.append(save_single_yaml_file(prefix_file, prefix_data))
        
        for suffix, records in suffix_dict.items():
            suffix_file = prefix_dir / f"{suffix}.yaml"
            suffix_data = [record.model_dump() for record in records]
            tasks.append(save_single_yaml_file(suffix_file, suffix_data if len(suffix_data) > 1 else suffix_data[0]))
