"""Feather format save functions."""

import asyncio
from pathlib import Path
from typing import List, Dict

try:
    import pyarrow as pa
    import pyarrow.feather as feather
except ImportError:
    pa = None
    feather = None



from scripts.models import JapanPostalCode
from scripts.progress_manager import get_progress_manager


async def save_feather(data: List[JapanPostalCode], output_path: Path) -> None:
    """Save data as Feather/Arrow file."""
    
    if feather is None or pa is None:
        print("Error: pyarrow is not installed. Install it with: pip install pyarrow")
        return
    try:
        # Get progress manager
        progress_mgr = get_progress_manager()
        task_id = None
        if progress_mgr:
            task_id = progress_mgr.add_task("all_data.feather", total=1)
        # Convert pydantic models to list of dictionaries
        data_dict = [item.model_dump() for item in data]
        
        def _save_feather():
            # Convert to Arrow table
            table = pa.Table.from_pylist(data_dict)
            # Write to Feather file
            feather.write_feather(table, output_path)
        
        await asyncio.to_thread(_save_feather)
        
        if progress_mgr and task_id is not None:
            progress_mgr.update("all_data.feather", advance=1)
    except Exception as e:
        print(f"Error saving Feather: {e}")
        import sys
        sys.exit(1)


async def save_grouped_feather(grouped_data: Dict[str, Dict[str, List[JapanPostalCode]]], datasets_dir: Path) -> None:
    """Save grouped data as Feather files with hierarchical structure."""
    if feather is None or pa is None:
        print("Error: pyarrow is not installed. Skipping grouped Feather files.")
        return
    
    
    semaphore = asyncio.Semaphore(50)  # Lower limit for Feather due to memory usage
    total_files = sum(1 + len(suffix_dict) for suffix_dict in grouped_data.values())
    
    # Get progress manager
    progress_mgr = get_progress_manager()
    task_id = None
    if progress_mgr:
        task_id = progress_mgr.add_task("Feather", total=total_files)
    
    async def save_single_feather_file(file_path: Path, records_list):
        async with semaphore:
            def _save():
                file_path.parent.mkdir(parents=True, exist_ok=True)
                records = records_list if isinstance(records_list, list) else [records_list]
                data_dict = [r.model_dump() if hasattr(r, 'model_dump') else r for r in records]
                table = pa.Table.from_pylist(data_dict)
                feather.write_feather(table, file_path)
            await asyncio.to_thread(_save)
            if progress_mgr and task_id is not None:
                progress_mgr.update("Feather", advance=1)
    
    tasks = []
    
    for prefix, suffix_dict in grouped_data.items():
        prefix_dir = datasets_dir / prefix
        prefix_dir.mkdir(parents=True, exist_ok=True)
        
        all_prefix_records = []
        for suffix_records in suffix_dict.values():
            all_prefix_records.extend(suffix_records)
        
        prefix_file = datasets_dir / f"{prefix}.feather"
        tasks.append(save_single_feather_file(prefix_file, all_prefix_records))
        
        for suffix, records in suffix_dict.items():
            suffix_file = prefix_dir / f"{suffix}.feather"
            tasks.append(save_single_feather_file(suffix_file, records if len(records) > 1 else records[0]))
    
    await asyncio.gather(*tasks)
    # Progress bar will be closed by main progress manager

